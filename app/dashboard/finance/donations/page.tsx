...
import { SimpleDonationStatistics } from "@/components/finance/donation-statistics"
...
export default function DonationManagement() {
  ...
  useEffect(() => {
    ...
    const filtered = donations.filter((donation: Donation) => {
      const matchesSearch =
      ...
      return matchesSearch && donation.status.toLowerCase() === activeTab.toLowerCase()
    })
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center">
        ...
        {is_DB_admin && (
          <div className="mt-4 md:mt-0">
            <AddEditDonationDialog onSuccess={refetch} />
          </div>
        )}
      </div>

      <SimpleDonationStatistics />
      ...
      <div className="flex flex-col md:flex-row gap-4 items-center">
        ...
        <Tabs defaultValue="all" className="w-full md:w-auto" onValueChange={setActiveTab}>
          <TabsList>
            ...
            <TabsTrigger value="failed" disabled={isLoading}>
              Failed
            </TabsTrigger>
            <TabsTrigger value="refunded" disabled={isLoading}>
              Refunded
            </TabsTrigger>
          </TabsList>
        </Tabs>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6">
          {filteredDonations?.map((donation) => (
            <DonationCard key={donation.id} donation={donation} onUpdate={refetch} />
          ))}
        </div>
      ) : (
        <div className="text-center py-12">
