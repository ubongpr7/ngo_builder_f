...
export function ProjectUpdates({ projectId, isManager, is_DB_admin, isTeamMember }: ProjectUpdatesProps) {
  ...
  // API queries
  const { data: updates = [], isLoading, refetch, isFetching } = useGetUpdatesByProjectQuery(projectId)
  ...
  const toggleUpdateExpansion = (updateId: number) => {
    setExpandedUpdates((prev) => (prev.includes(updateId) ? prev.filter((id) => id !== updateId) : [...prev, updateId]))
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between gap-4">
        ...
        <div className="flex gap-2">
          ...
          {(isManager || isTeamMember) && (
            <Button className="bg-green-600 hover:bg-green-700 text-white" onClick={() => setAddUpdateOpen(true)}>
                      ...
                      <div className="flex justify-between items-start">
                        <div>
                          ...
                          <CardDescription className="flex items-center gap-1">
                            ...
                            Submitted by {update.submitted_by_details?.first_name}{" "}
      ...
      {selectedMedia && <ViewMediaDialog media={selectedMedia} open={viewMediaOpen} onOpenChange={setViewMediaOpen} />}
      ...
      <UpdateStatisticsDialog projectId={projectId} open={statisticsOpen} onOpenChange={setStatisticsOpen} />
    </div>
  )
}
